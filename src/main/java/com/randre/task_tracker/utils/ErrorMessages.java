package com.randre.task_tracker.utils;

public class ErrorMessages {
    public static final String USER_NOT_FOUND = "Usuário não encontrado";
    public static final String USER_ALREADY_EXISTS = "Usuário já existe";
    public static final String TASK_NOT_FOUND = "Tarefa não encontrada";
    public static final String ACCESS_DENIED = "Usuário não possui permissão";

    public static final String DATE_RANGE = "A data de término deve ser maior ou igual à data de início";
    public static final String FUTURE_DATE = "A data de início / data de término deve ser maior do que a data atual";

    public static final String MAXIMUM_NAME_SIZE = "O nome deve possuir no máximo 30 caracteres";
    public static final String MAXIMUM_TITLE_SIZE = "O título deve conter no máximo 50 caracteres";
}
